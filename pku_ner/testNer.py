...
class testNer: #Named Entities Recognition
    ...
    correctTotal = 0        # correctTotal
    precisionTotal = 0      # precisionTotal
    recallTotal = 0         # recallTotal
    precisionRate = 0       # precisionRate
    recallRate = 0          # recallRate
    F = 0                   # F
        ...
        for sent in self.testSents:
            ...
            for pair in sent:
                ...
                if index == 0:
                    # 首节点
                    wNode = wordNode(word,[])
                    if word in self.wordPosFreq.keys():
                        # 该词在词典中
                        if word in self.wordPosHeadPro.keys():
                            # 当过句首
                    ...
                    if word in self.wordPosFreq.keys():
                        ...
                        for p in posList:
                            ...
                            for pn in prePosNodeList:
                                ...
                                if prePos in self.posTransPro.keys():
                                    if p in self.posTransPro[prePos].keys():
            ...
            for pair in sent:
                ...
                if bestPath[index] != 'N':
                    self.precisionTotal += 1
                if pos == bestPath[index] and pos != 'N':
                    self.correctTotal += 1
            output.write('\r\n')

        output.close()

        self.precisionRate = 1.0 * self.correctTotal / self.precisionTotal
        self.recallRate = 1.0 * self.correctTotal / self.recallTotal
        self.F = 2 / ( 1/self.precisionRate + 1/self.recallRate )

        print('correctTotal:',self.correctTotal)
        print('recallTotal:',self.recallTotal)
        print('precisionTotal:',self.precisionTotal)
        print('precision rate:',self.precisionRate)
        print('recall rate:',self.recallRate)
        print('F值:',self.F)